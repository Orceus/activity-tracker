...
# ── File logging (visible even with console=False / silent mode) ─────────────
def _setup_logging():
    """Set up file-based logging so we can debug production issues."""
...
def _write_last_alive():
    ...
    try:
        ...
        alive_path.write_text(datetime.now().isoformat())
    except Exception:
        pass
...
class OptimizedDataSyncer:
    ...
    def __init__(self, supabase_url=None, supabase_key=None):
        ...
        # NEW: Track current inactive state for continuous batching
        ...
        while self.is_syncing:
            try:
                ...
                if current_time - self.last_sync_time >= self.sync_interval:
                    ...
                    # Update last_alive after successful sync
                    _write_last_alive()
...
class ActivityTracker:
    def __init__(self, silent_mode=True, supabase_url=None, supabase_key=None):
        ...
        self.data_syncer = OptimizedDataSyncer(supabase_url, supabase_key)
        self.last_sync_time = time.time()
    ...
    def track_activity(self):
        ...
        while self.is_tracking:
            try:
                ...
                logging.error("Error in tracking loop: %s", e, exc_info=True)
                if not self.silent_mode:
